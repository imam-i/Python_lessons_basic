...
для web-сервисов и мобильных приложений. Архивные данные доступны только на коммерческой основе. В качестве источника
данных используются официальные метеорологические службы, данные из метеостанций аэропортов, и данные с частных метеостанций.
...
def geocode(geocode):
    ...
    session.params = {
        'apikey': '04bf1a11-02ec-4d24-8797-9f8efcdf1105',
        'geocode': geocode,
        'format': 'json'
    }
    _json = session.get('https://geocode-maps.yandex.ru/1.x/').json()
    return _json['response']['GeoObjectCollection']['featureMember'][0]['GeoObject']['Point']['pos']

def weather(city):
    lon, lat = geocode(city).split()
    ...
    session.headers['X-Yandex-API-Key'] = 'd95e7755-0361-4e02-98d9-13bac6fcd2c7'
    get = session.get('https://api.weather.yandex.ru/v1/forecast')
    return get.json()

for _geocode in 'Москва', 'Санкт-Питербург':
    _weather = weather(_geocode)
    for itm in _weather:
        print(itm, _weather[itm])


# for key, value in weather('Москва').items:
#     print(key, value)
#
# for key, value in weather('Махачкала').items:
#     print(key, value)